...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
     #pragma omp parallel
	{
		...
		int modSize = omp_get_num_threads();
		int iPlus;

		//Start with i as 0 and advance i by the number of threads being used
		for(int i=0; i<ARRAY_SIZE; i+=modSize) {
			//Similar intuition to using a mod function
			iPlus = i + omp_get_thread_num();
			if (iPlus < ARRAY_SIZE) {
				z[iPlus] = x[iPlus] + y[iPlus];
			}
		}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
	#pragma omp parallel
	{
		//Getting how big the block each thread is responsible for
		int blockSize = ARRAY_SIZE/omp_get_num_threads() + 1;
		int iPlus = blockSize * omp_get_thread_num();

		for(int i=0; i<blockSize; i++) {
			if (i + iPlus < ARRAY_SIZE) {
				z[i + iPlus] = x[i + iPlus] + y[i + iPlus];
			}
		}
