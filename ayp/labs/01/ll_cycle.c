...
int ll_has_cycle(node *head) {
	//Creating two pointers to point to current node
    node *tortoise = head;
    node *hare = head;
    
    //While neither is null
    while (tortoise != NULL && hare != NULL) {
        //Advance both pointers once
        hare = hare -> next;
        tortoise = tortoise -> next;
        
        //Ensure that you can advance hare one more time (if you can't, need to exit to prevent a segmentation fault)
        if (hare == NULL) {
            return 0;
        }
        
        //Hare should move two pointers along
        hare = hare -> next;
        
        //They are pointing at the same thing... Cyclical
        if (tortoise == hare) {
            return 1;
        }
    }
    
    //One of them became null before they both pointed at each other, they are not cyclical
    return 0;
}
