...
inst_needs_relocation:
        #get the opcode of the 32 bit instruction
        srl $t0, $a0, 26
        #check whether matches jump
        addiu $t1, $zero, 0x2
        beq $t0, $t1, return_needs_relocation
        #check whether matches jump and link
        addiu $t1, $zero, 0x3
        beq $t0, $t1, return_needs_relocation
        #0 other wise
        addu $v0, $zero, $zero
        j just_return
return_needs_relocation:
        addiu $v0, $zero, 1
just_return:
        jr $ra
...
relocate_inst:
        ...
        #look up the current offset in the relocation table
        addu $a0, $zero, $s3
        addu $a1, $zero, $a1
        jal symbol_for_addr
        beq $v0, $zero, return_error
        #get the absolute address of the label in the symbol table 
        addu $a0, $zero, $s2
        addu $a1, $zero, $v0
        jal addr_for_symbol
        beq $v0, $zero, return_error
        #shift absolute address 2 bits to the right, then clear upper 6 bits
        srl $v0, $v0, 2
        andi $v0, $v0, 0x03ffffff
        #clear the lower 26 bits of instruction
