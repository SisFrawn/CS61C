...
hex_to_str:
	...
	#start with first four bits
	addu $t2, $zero, $zero #counter, increment by 4
loopH:
	addu $t0, $zero, 28
	subu $t0, $t0, $t2
	slt $t1, $t0, $zero
	bne $t1, $zero, returnH #if $t0 is now less than zero
	srlv $s0, $a0, $t0
	andi $s0, $s0, 0x0000000f #mask with 0xf
	slti $t1, $s0, 10 #check whether the byte is a digit or a letter
	bne $t1, $zero, digit
letter:
	addiu $s0, $s0, 87
	j write
...
write:
	...
	addu $t3, $a1, $t4
	sb $s0, 0($t3)
	addiu $t2, $t2, 4 #increment our counter
	j loopH
returnH:
	#store new line
	li $t0, '\n'
