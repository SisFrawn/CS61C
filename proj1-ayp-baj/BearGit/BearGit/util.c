...
//cryptohash seems to take an arbitrary length str (perhaps a combination of branch and previous hash) and an array where the new hash will be placed (must keep a pointer to the array passed in
void cryptohash(const char* str, char dst[SHA_HEX_BYTES + 1]) {
    
    //Initializes a buffer of 20 + 1 bits (null terminated)
    char buf[SHA_DIGEST_LENGTH];
    
    //Calls the hash function on str
    SHA1((unsigned char*)str, strlen(str), (unsigned char*) buf);
    
    //Copies over the hash result from the buffer into the passed-in array
    for (size_t i = 0; i < SHA_DIGEST_LENGTH; ++i) {
        sprintf(&dst[i*2], "%02x", (unsigned char)buf[i]);
    }
    //Null terminates the array that was passed in
    dst[SHA_HEX_BYTES] = '\0';
}
